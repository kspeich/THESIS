...
class DirectorField:
    ...
    def plot_n(self, t):
        ...
        x, y = np.meshgrid(np.arange(self.length), np.arange(self.width))
        ax.streamplot(x, y, np.transpose(self.n[t][0]), np.transpose(self.n[t][1]), density=5, linewidth=0.5, arrowsize=1e-10, color='b')
        fig.suptitle(f'Director field from {self.name}, t = {t}', fontsize=18)
    ...
    def plot_defect_density(self, cutoff, r):
        ...
        area = (self.length - 1) * (self.width - 1)                                                                 # Area of the lattice
        defect_density = [len(defects[t]) / area for t in range(self.timepoints)]
    ...
    def defect_density(self, cutoff, r):
        defects = self.find_all_defects(cutoff, r)
        area = (self.length - 1) * (self.width - 1)                                     # Area of the lattice
        defect_density = [len(defects[t]) / area for t in range(self.timepoints)]       # List of defect densities by time
        
        return np.mean(defect_density)                                                  # Average over time
...
def new_row(director, inferences, cutoff, r, nPairs):
    inferences = pd.read_csv(f'{Path(inferences) / director.name}.csv')
    return pd.DataFrame([{
        'filename': Path(file).stem, 
        'k': np.mean(inferences['k']), 
        'z': np.mean(inferences['z']), 
        'defect_density': director.defect_density(cutoff, r), 
        'correlation_length': director.correlation_length(nPairs)
        }])

def plot(df):
    ratio = df['z'] / df['k']

    fig, (ax_d, ax_l) = plt.subplots(1, 2, figsize=(15, 6))
    ax_d.scatter(ratio, df['defect_density'])
    ax_l.scatter(ratio, df['correlation_length'])

    fig.suptitle('Defect density and correlation length by inferred parameters', fontsize=18)
    ax_d.set_xlabel(r'$\alpha / K$', fontsize=16)
    ax_l.set_xlabel(r'$\alpha / K$', fontsize=16)
    ax_d.set_ylabel('Defect density', fontsize=16)
    ax_l.set_ylabel('Correlation length', fontsize=16)

    return fig, (ax_d, ax_l)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--directory', type=str, default='../../../data/processeddata')                                     # Directory of the datasets
    parser.add_argument('--inferences', type=str, default='../../../data/inferences/ConvNextParameterEstimator_KZ_v3')      # Directory with inferences generated by infer_parameters.py
    parser.add_argument('--outputdir', type=str, default='../../../data/analysis')                                          # Output directory of results
    parser.add_argument('--cutoff', type=float, default=0.7)                                                                # The cutoff in S to identify defect candidates
    ...
    parser.add_argument('--nPairs', type=int, default=10000)                                                                # Number of pairs to sample when finding correlation length
    ...
    df = pd.DataFrame(columns=['filename', 'K', 'Z', 'defect_density', 'correlation_length'])
    
    for file in files:
        logger.info(f'Analyzing file {file.name}')
        director = DirectorField(file, args.size)

        df = pd.concat([df, new_row(director, args.inferences, args.cutoff, args.r, args.nPairs)], ignore_index=True)

        # director.plot_n(0)[0].savefig(f'{args.outputdir}/Director{director.name}_t=0.png')
    ...
    plot(df)[0].savefig(f'{args.outputdir}/DefectDensityCorrelationLength.png')

    # Save the as a CSV file so we can access later
    df.to_csv(f'{args.outputdir}/DefectDensityCorrelationLengths.csv')